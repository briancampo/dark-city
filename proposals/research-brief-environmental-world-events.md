...
Should Dark City add a periodic, no-citizen-actor producer of "outside the walls" content — weather, ambient change from agent action, and news/events from beyond the simulated town — so that the world feels less like a sealed box, citizens have something ambient to notice, and the Narrator has something new to report beyond what citizens themselves discuss/generate?
...
Emergence World ties its agents to _real_ Earth weather and _real_ news because its research goal is studying how agents behave when exposed to genuinely unauthored, unpredictable external signal. Dark City's stated goal (Design Foundations §1) is different: "populate constructed worlds... first for our own study, eventually as the substrate a player steps into" — and §10's sculptable-worlds principle means a given scenario package might not be contemporary-Earth-like at all. Piping real weather.gov data into a fictional 1920s-noir scenario, for example, would be a mismatch, not a feature.

**Recommendation: procedurally/LLM-generated fictional content, steered by scenario-level configuration, not a live external API integration.** Real-world API grounding remains a distinct, larger, and explicitly _un_-adopted option here — parallel to how Decision 0003 explicitly deferred (rather than left ambiguous) an optional cross-world interaction mechanism. If live grounding is ever wanted for a specific scenario, that's a separate future decision, not a default of this one.

## Two Candidate Shapes

**Option A — The Narrator wears a second hat.** Extend the existing Narrator persona (§8.1) with an additional daily pass (separate from its existing 8-in-game-hour edition cadence) that generates weather/internal and external affairs content directly.

**Option B — A distinct, dedicated persona.** A new non-autonomous, invisible system persona (call it, provisionally, "Animus") separate from the Narrator, with its own timed trigger, writing directly to `world_events` with a new `source` value. The Narrator then picks these rows up exactly the way it already picks up any other `world_events` row (§8.2 step 1's existing query needs no change).

**Recommendation: Option B.** This keeps each system persona's job conceptually singular — the Narrator's job is "report on what happened" (§8.1), not also "decide what the weather is." Animus' job is to inject interesting and meaningful events into the world as a distinct producer. It's also consistent with the precedent this same review cycle already set: the Tool Review Agent (Decision 0006) is its own persona doing its own singular job (audit code), not folded into an existing one. One persona per responsibility, communicating only through the tables both already read/write, keeps the pattern uniform.
...
- **A timed cadence trigger** configured in the scenario package and running inside the same per-world headless tick that already drives the Narrator's cron-style trigger (Backlog 2.3.2) and the scripted-event scheduler (2.6.1) — no new scheduling mechanism needed, just a new consumer of the existing tick.
...
- **Salience assigned by the Animus persona at creation**, same convention as every other producer under Decision 0004 — no special case.
...
Immediate assessment for any Phase 1 impacts.
Full Research proposed to be conducted during Phase 2 with proposal review to Steward and User.

### Decision

Decision requires immediate sequencing assessment may be necessary in Phase 1.

### Implementation

Implementation Sequenced in **Phase 2** (not Phase 3) since it's a natural extension of already-Phase-2 systems (Narrator, Epic 2.3; world-event production, Epic 2.6) rather than depending on Phase 3 maturity. Implementation stories (2.7.2 onward) get drafted only after 2.7.1's decision is ratified.
Once implementation exists, Phase 2 Exit Criteria should gain a bullet confirming at least one environmental event reached both a citizen (via Observation) and a Narrator edition — not added yet, since only the spike is being sequenced here.

Proposed **Epic 2.7 — Environmental & External World Events**, Blueprint § TBD (assigned once ratified):
