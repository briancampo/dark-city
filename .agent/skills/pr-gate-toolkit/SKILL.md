...
- [ ] **`cargo xtask check` clean.** The project-specific lint/rule suite (or run `scripts/gh-task-ops.sh check`).
...
- [ ] **Architecture & Blueprint alignment.** Implementation honors existing Architecture, Project Standards, and Blueprint specs without speculative generality.
...
- [ ] **Handoff note included**, even if the ticket is fully done — Session Start Workflow step 10. (Running `scripts/gh-task-ops.sh pr-create` automatically attaches the DoD checklist and handoff template).
