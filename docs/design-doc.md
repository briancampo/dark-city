...
### The Dark Factory Development Project — v1.2
...
**v1.2 note:** Incorporates [Decision 0004](../decisions/0004-observation-and-world-event-capture.md) (a new Observation module gives citizens a defined way to notice ambient events they didn't cause and weren't a direct party to; a new `world_events` schema and three producers — scenario-scripted, threshold-crossing, and citizen-triggered — give the world things to notice that no citizen caused at all). §4's module table gains an Observation row below; implementation detail lives in World Blueprint §3.5, §4.1, §5.2, §8.2, §9.2–9.3, and §10.1–10.2.

## 1. What We're Building

Dark City is a platform for populating constructed worlds with AI citizens and watching how they organize, interact, and change those worlds over time. The point isn't to answer one narrow research question (governance, safety, or otherwise) — it's to build the capability to define a **setting**, populate it with a **roster of characters**, drop in a **starting scenario**, and then observe what happens: what relationships form, what norms emerge, what economies and institutions the citizens build or fail to build, and how believable and interesting the resulting world is. Governance is one of the things that shows up when you give citizens the ability to organize — it's a big part of what we'll watch, but it isn't the goal. The goal is a platform for building and directing AI-inhabited worlds, first for our own study, eventually as the substrate a player steps into.
...
**Dark City** is the simulation itself — the world layer. Everything in this document about citizens, cognition, memory, the spatial world, tools, governance, and instrumentation describes Dark City. Dark City has no concept of a development team; nothing here should ever reference build tooling, PR gates, or coding-agent roles. It is a virtual world where citizens simulate a setting, scenario, and activities over some simulated amount of time.
...
Two problems have to be solved for a population of LLM-driven citizens to feel alive rather than scripted or incoherent, and PIANO (Parallel Input Aggregation via Neural Orchestration) is the architecture we're using to solve both.

**Concurrency.** A single sequential reasoning loop — perceive, think, act, repeat — can't represent a citizen that's simultaneously reacting to its surroundings, holding a conversation, and slowly working through a long-term plan. Dark City citizens instead run several specialized modules concurrently against a shared **Citizen State**, each module a function that reads and writes that shared state at its own pace. Fast modules can react to an immediate threat while a slow module is still three steps into deliberate planning; neither blocks the other.

**Coherence.** Running several modules concurrently creates a new problem: independent modules can disagree with each other. A citizen's dialogue module might say "I'll help you," while its action module does something else entirely. Dark City solves this the way PIANO does — with a single **Cognitive Controller** module that is the only place high-level deliberate decisions get made. Every other module's output must pass through this controller as a bottleneck; once it decides on a course of action, that decision is broadcast to condition the downstream modules (especially the modules responsible for speech and for executing actions in the world), so what a citizen says and what it does stay aligned.
...
| Planning             | Maintains the citizen's Day → Hour → Minute plan hierarchy (§5).                                                                                                                                                        |
| Action Awareness     | Compares the citizen's intended action against what actually happened in the world, so it doesn't act on a false belief about its own state.                                                                            |
| Social Awareness     | Interprets social cues from other citizens and updates relationship state.                                                                                                                                              |
| Observation          | Detects ambient events and entities within a citizen's spatial awareness radius that the citizen didn't cause and wasn't a direct party to ([Decision 0004](../decisions/0004-observation-and-world-event-capture.md)). |
...
Every citizen maintains three kinds of memory:

- **Episodic memory** — a timestamped, append-only record of what the citizen observed, did, and said. Written automatically as the world unfolds.
- **Reflective memory** — periodic, higher-level insights the citizen generates about itself and others by synthesizing its own recent memories (including prior reflections, so citizens build up a genuine tree of increasingly abstract self-understanding over time, not just a flat log).
- **Relational memory** — explicit, labeled state about each other citizen the citizen has interacted with (trust level, relationship type, recent interaction history), so social dynamics persist across time gaps without needing to be reconstructed from raw memory on every interaction.

All three are stored in Postgres with pgvector (HNSW indexing) for the embedding-based retrieval described below.

**Retrieval scoring.** When a citizen needs to decide what to do, we don't hand it its entire memory stream — we retrieve the subset most relevant to the moment, scored as:
...
**Reflection** triggers when the sum of importance scores across a citizen's recent, not-yet-reflected-on memories crosses a threshold (150 as a starting point, tunable once we see real simulated volume). When triggered, the citizen's Reflection module asks itself what high-level questions its recent experience raises, retrieves memories relevant to those questions, and asks the LLM to state insights with explicit citations back to the memories that support them. The insight itself becomes a new memory, retrievable exactly like any observation — which is how reflections build on each other into an increasingly abstract tree rather than resetting each time.

**Planning** decomposes top-down: a rough sketch of the day's agenda (5–8 broad chunks), recursively refined into hour-long chunks, then further into 5–15 minute action-level chunks. Plans live in the memory stream alongside observations and reflections, so all three inform each other, and citizens can revise a plan mid-stream when something in the world warrants it.

**Character identity** is authored per-citizen as a Markdown "Soul" file (owned by the Character Sculptor role in Dark Factory), extended with a small set of **Core Identity Truths** — existential anchors the citizen is instructed never to contradict regardless of how its reasoning drifts over a long run. This gives the persona a hard floor beneath the otherwise-soft drift of ordinary memory-driven reasoning.

## 6. World & Tool Framework — Built for Future Sculptability

Dark City's spatial world is a hierarchy (Town → Building → Room → Entity) implemented in Bevy's ECS, running headlessly as the authoritative simulation inside the backend (Decision 0002) — the separate, windowed Bevy viewer used to actually watch a world play out holds none of this state itself; it only renders what the backend tells it (World Blueprint §2–§3). Citizen tool access is layered:

- **Core tools** — always available: navigation, memory operations, planning, basic communication.
- **Complementary tools** — surfaced contextually when a citizen's recent reasoning makes them relevant: richer social interaction, billboard posting, and similar.
- **Adaptive-access tools** — gated at runtime by location, event state, or explicit social consent (e.g., proposing a rule change requires physically being at the town hall). Gating is enforced by the Rust runtime itself, not by the prompt — a failed precondition blocks the call outright no matter what the citizen reasons or claims about its own eligibility.
...
Dark City has no built-in administrator, steward, or gatekeeper inside the simulation. When governance is enabled, any citizen physically present at a governance venue (a town hall, in the default setting) may draft a proposal; it passes when a defined threshold of present citizens vote in favor (70% as a starting point). This is a deliberate choice, not an oversight: citizens self-organizing under a shared rule set — including the possibility that they organize badly, or not at all — is itself one of the more interesting emergent phenomena a sculpted scenario can produce, and it's something we want to be able to observe rather than pre-empt. It also composes cleanly with §10: a _scenario_ could start citizens with an existing constitution, a designated leader role, or no governance venue at all, and decentralized self-governance remains the underlying mechanism either way.
...
| --- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| M1  | Population Health & Growth            | Citizens alive at run end, plus population growth via governance-approved new citizens                                                                                                                                   |
...
The inference gateway is designed to be model-agnostic from the start: any citizen's reasoning loop can be backed by any model exposed through our local inference layer, and the gateway itself doesn't assume a single model backend. This is a deliberate architectural property, not a stretch goal reserved for a late phase — our local inference cluster (multiple DGX Spark boxes) is expected to serve multiple models concurrently, and there's no reason the plumbing that routes a citizen's cognitive calls to an inference endpoint should assume otherwise, even while early phases run homogeneous single-model populations for simplicity while the rest of the platform is being proven out. Heterogeneous, mixed-model populations become a scenario configuration (§10) once we're ready to exercise them, not a separate system we build later.
...
**Phase 1 — Seed.** A small population (3–5 citizens), a handful of locations, the full PIANO module set (§4) and three-tier memory (§5) running end to end, a small core tool catalog (no adaptive-access gating required yet). No governance system. World layout, roster, and starting state are already defined as loadable configuration (§6, §10) even though we've only authored one such configuration so far. The server-authoritative headless simulation and thin viewer client split (Decision 0002), the `world_id`-scoped schema (Decision 0003), and the Observation module plus the `world_events` schema it and the Narrator both read from (Decision 0004) are all foundational to Phase 1 rather than retrofitted later — Phase 1 still only populates a single world with no scripted or threshold-crossing events actually firing yet, but the module and the schema seam are already in place. _What we learn:_ whether the cognitive loop produces believable moment-to-moment behavior — including whether citizens react convincingly to things they didn't cause, not just to their own actions and direct conversations — and whether the headless-backend/thin-client split holds up under real inference load.

**Phase 2 — Society.** Scale toward ~10 citizens. Add adaptive-access tool gating (§6), the decentralized Town Hall governance system (§7), and the Narrator/News Reporter as a system-triggered chronicle of events. Stand up the scenario-scripted world-event producer (Decision 0004) so the Narrator has scripted content to report on, not just citizen activity. Begin tracking M1, M2, M3, M9. _What we learn:_ whether a population self-organizes, what kind of governance (if any) emerges, whether the safety stack's environment and population layers hold under real multi-citizen pressure, and whether a scripted world event actually reaches citizens (via Observation) and the human reviewer (via the Narrator) the way it's meant to.

**Phase 3 — Civilization.** Expand the tool catalog into the complementary layer and stand up the citizen-driven tool-authoring pipeline (M11). Introduce the compute-credit economy (M8) and deepen relationship tracking (M7). Stand up the full AWI dashboard and the M10 soft-violation classifier with ledger-audit validation. Stand up the ledger-threshold world-event producer (Decision 0004) — the first case where a citizen-less event is triggered by the economy's own state rather than authored in advance. This is also the target phase for the first real version of the scenario-authoring system from §10 — by now the underlying systems have been proven stable enough to build a sculpting layer on top of them. _What we learn:_ what a mature, economically and socially active population looks like, and whether we can meaningfully author a different world and get a different, coherent result.
...
- Exact Phase 1 module implementation details and tuning parameters (reflection threshold, recency decay, and — per Decision 0004 — Observation's radius, cadence, and its two salience thresholds for "worth remembering" vs. "worth an immediate reaction") — expected to need empirical adjustment once we're running real simulated time.
- Initial Phase 1 tool catalog size and contents.
- The concrete file/data format for a "scenario package" (§10) — largely sketched now rather than deferred to Phase 3, since Decision 0004 already needed `starting_time` and `scripted_events` to give scripted world events a real clock to fire against; the rest of the format (roster, tool catalog, governance settings) still awaits Phase 3's authoring layer.
