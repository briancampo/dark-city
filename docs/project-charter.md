...
| ------------------------------ | --------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Steward**                    | _(coordinates; owns no code directly)_                                                        | —                            | Ticket dispatch, decision log maintenance, PR gating (process), cross-boundary arbitration. Wires the Tool Review Gate into the `new_tool` passage handler ahead of runtime registration.              |
...
| **Local Inference Specialist** | `crates/dark_city_inference/`, `grammars/`                                                    | §3.3 (inference side), §11   | Inference gateway integration, grammar/schema enforcement, multi-model routing. Implements the Tool Review Agent persona and its grammar-enforced verdict schema                                       |
