...
| 5   | [Session Start Workflow](.agent/workflows/start-session.md)            | Dark Factory | Step-by-step bootstrap checklist run at the start of every session                                                                                                              | Done   |
...
| 10  | [Decision 0004](decisions/0004-observation-and-world-event-capture.md) | Dark Factory | Observation module for ambient/third-party awareness (with an urgency path to the Cognitive Controller); `world_events` schema and three producers for citizen-less events      | Done   |
...
Epic 1.0 (repository & tooling bootstrap) is complete, including containerized backend deployment. Before Epic 1.1 goes further, three architecture decisions were made and are recorded in the decision log: [Decision 0002](decisions/0002-server-authoritative-simulation.md) moves all simulation state and PIANO module execution into a headless backend ECS App, with the Bevy client reduced to a thin, stateless viewer (`dark_city_client`); [Decision 0003](decisions/0003-multi-tenant-world-instances.md) adds isolated, multi-tenant world instances (a `worlds` table and `world_id` scoping) ahead of Phase 3/4 needing them; [Decision 0004](decisions/0004-observation-and-world-event-capture.md) adds a new Observation module so citizens can notice ambient events they didn't cause and weren't a direct party to (with an urgency path to the Cognitive Controller for genuinely time-sensitive ones), plus a `world_events` schema and three producers — scenario-scripted, threshold-crossing, and citizen-triggered — for events no citizen caused at all. The Blueprint, Design Foundations, Team Charter, AGENTS.md, and Backlog have all been updated to match — the natural next step is dispatching the revised Epic 1.1 tickets (1.1.1 now includes the `world_events`/`observable_events`/salience schema work alongside the `worlds` table) before proceeding into Epic 1.2, which now includes a new Observation module story.
...
- The one open architectural item worth remembering going into the Backlog: Design Foundations §14 lists a few parameters (reflection threshold, recency decay, Observation's radius/cadence/salience thresholds, Phase 1 tool catalog size) that are expected to need empirical tuning once there's a real running system to observe. §14 also now flags a lightweight browser-based observer/spectator view and an optional cross-world interaction mechanism as raised-but-not-yet-scoped ideas — worth a look before they're needed rather than after.
